...
int ADPi::TransReceProcess(unsigned char channel, int *length, char *buf){
		...
		*(buf+len) = '\0';
		...
		if((tmpBuf[1] == 0xc9)||(tmpBuf[2] == 0xa3)){
			*length = len;
			return 1;
		}
		if((tmpBuf[1] == 0xb6)||(tmpBuf[2] == 0xdc)){
			*length = len;
			return 0;
		}
...
int raspberryPinNumber(int arduinoPin){
	switch(arduinoPin){
		...
		case 4: if(REV == 1){return 21;}else{return 27;} break;
